<script>
  ...
  let isLoading = false
  $: canBeSaved =
    user.mobile === newMobile && user.fullName === newName && !newPassword
      ? false
      : newMobile &&
          newMobile.length == 10 &&
          newName.length > 2 &&
        ...
        ? true
        : false
  $: displayWrongPassword = newPassword
  ...
  const handleMobileChange = ({
    detail: {
      inputState: { unmaskedValue: mobile },
    },
  }) => (newMobile = mobile)
  const handleUpdateUser = () => {
    ...
    axios({
      ...
      .catch((err) => {
        ...
        addNotification(err.response?.data, 8000)
      })
  }
</script>
...
<div class="settings-wrapper">
  <form class="settings" on:submit|preventDefault={handleUpdateUser}>
    <div class="settings-prop">
      ...
      <span
        use:tooltip
        title="Служит к отображению ваших данных на странице объекта. Не может быть изменён."
        class="value tooltip"
      >
        {agentIdentifier}
        <svg
    ...
    <div
      class="settings-prop"
      style="border-top: 1px dashed rgb(51, 51, 51);padding-top: 1.5em;margin-top: 1.5em;"
    >
      <label for="old-password" class="label">Старый пароль</label>
      <input
        required={newPassword && !displayWrongPassword ? true : false}
        type="password"
        id="old-password"
        class="value"
  ...
  <div class="information">
    <details>
      ...
      <ul>
        <li>
          <h4>
            Мин. и макс. значения в фильтрах на главной динамические, зависят от
            ваших объявлений
          </h4>
        </li>
        <li>
          <h4>Цена объекта в неосновных валютах зависит от курса валют</h4>
        </li>
        <li><h4>Основная валюта объявления задаётся при его создании</h4></li>
        <li>
          <h4>
            Заданные пользователем филтры не влияют на секцию лучших предложений
          </h4>
        </li>
        <li>
          <h4>Объявления отсортированы по дате создания от нового к старому</h4>
        </li>
        <li>
          <h4>Уже сдано: не отображается в топе, прямая ссылка работает</h4>
        </li>
        <li>
          <h4>
            Уже сдано и дата освобождения не позднее месяца от текущей даты:
            отображается с предупреждением
          </h4>
        </li>
        <li><h4>Уже сдано и дата освобождения в прошлом: отображается</h4></li>
        <li>
          <h4>
            Скрытое: не отображается на сайте, прямая ссылка для обычных
          ...
          <div class="information" style="margin-left: 1em;">
            <details>
              ...
              <ul>
                ...
                <li>
                  <h4>
                    Последнее успешной обновление: {dayjs(
                      changeRates.timestamp
