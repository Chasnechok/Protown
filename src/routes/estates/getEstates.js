...
export async function get(req, res) {
  ...
  const { courses: changeRates } = await Course.findOne()
  const fee =
    filters.included &&
    filters.included.some((el) => el.value === 'fee' && el.c)
      ? false
      : true
  const soonFree =
    filters.included &&
    filters.included.some((el) => el.value === 'soonFree' && el.c)
      ? true
      : false
  if (filters.included)
    filters.included = filters.included
      .filter((el) => el.value !== 'soonFree' && el.value !== 'fee' && el.c)
      .map((el) => el.value)
    ...
    await Estate.find(
      !filters.isInitial
        ? {
            ...
            'extras.included': filters.included[0]
              ? { $in: filters.included }
              : { $exists: true },
            isHidden:
              token && filters.displayHidden ? { $in: [true, false] } : false,
            $or: [
              {
                realised:
                  filters.displayHidden || !soonFree
                    ? { $exists: true }
                    : false,
              },
              {
      ...
      .sort({ createdAt: -1 })
      .then((r) => {
        const foundEstates = filters.isInitial
          ? r
          : r.filter((el) => {
              const a = currencyCalculator(
                el.price,
                filters.currency,
                el.currency,
                changeRates
              )
              return a >= 0 && a <= filters.maxPrice[0]
            })
