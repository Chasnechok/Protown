  ...
  try {
    ...
    await Estate.aggregate([
      ...
      .then((r) =>
        r.forEach(
          (el) =>
            (maxPricesPerCurrencyAndDeal[el.deal][el.currency] = el.maxPrice)
        )
      )
      .catch(console.log)
    ...
    await Estate.aggregate([
      {
        $match: {
          isHidden: false,
          $or: [{ realised: false }, { realised: { $lt: new Date(date) } }],
        },
      },
      {
        $group: {
      ...
      return send(res, 200, { maxPricesPerCurrencyAndDeal, ...r[0] })
    })
  } catch (error) {
    send(res, 500, error)
  }
