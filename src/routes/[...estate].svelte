...
<script>
  ...
  $: priceInWords = num.format(
    currencyCalculator(
      fetchedEstate.price,
      $currencyOnPage,
      fetchedEstate.currency,
      changeRates
    )
  )
  const images =
    fetchedEstate.images && fetchedEstate.images[0]
      ? fetchedEstate.images.map((el, i) => ({
          id: i,
          src: `https://assets.protown.in.ua/estates/${fetchedEstate.type}/${fetchedEstate._id}/${el}`,
        }))
      : undefined
...
<section>
  <div class="estate-content-wrapper">
      ...
      <div class="h-wrapper">
        ...
        <div class="type-deal-prop">
          {#if fetchedEstate.type}
            <span
              >{fetchedEstate.type === 'flat'
                ? 'Квартира'
                : fetchedEstate.type === 'house'
                  ? 'Дом'
                  : fetchedEstate.type === 'commersion'
                    ? 'Коммерция'
                    : 'Участок'}</span
            >
          {/if}
        ...
        {#if spravce}
          <div style="position: absolute;">
            <a
              target="_blank"
              rel="noopener"
              href="/adminka?mode=edit&id={fetchedEstate._id}"
    ...
    <div class="estate-wrapper">
      {#if mountedToDom && images && images.length === fetchedEstate.images.length}
        <div class="image-carousel" bind:this={imageCarousel}>
          <Swiper
            ...
            lazy={{
              loadPrevNext: true,
              elementClass: 'estate-image-lazy',
              loadedClass: 'estate-image-lazy-loaded',
              loadingClass: 'estate-image-lazy-loading',
              preloaderClass: 'estate-image-lazy-preloader',
            }}
            on:swiper={({ detail }) => {
              swiper = detail[0]
              if (imageCarousel)
                imageCarousel.style = 'display: flex; align-items: center;'
            }}
          >
              ...
              <SwiperSlide>
                <div class="image-wrapper">
                  <img
                    ...
                    data-src={estateImage.src}
                    alt="{fetchedEstate.type &&
                      fetchedEstate.type} image #{estateImage.id}"
      ...
      <div
        style={images ? (!swiper ? 'opacity: 0;' : '') : ''}
        class="estate-info-wrapper"
      >
        <div
          class="estate-price"
          class:crossed={fetchedEstate.realised &&
            (isNaN(Date.parse(fetchedEstate.realised)) ||
              dayjs(fetchedEstate.realised).isAfter(dayjs()))}
          title={priceInWords}
        >
          ...
          ><span class="price-currency crossed"
            >&nbsp{$currencyOnPage === 'USD'
              ? '$'
              : $currencyOnPage === 'EUR'
                ? '€'
                : '₴'}{fetchedEstate.deal === 'lease' ? ' / месяц' : ''}</span
          >
          {#if !fetchedEstate.extras.fee}<span class="crossed"
              >&nbspбез комиссии</span
            >{/if}
          {#if fetchedEstate.realised && (isNaN(Date.parse(fetchedEstate.realised)) || dayjs(fetchedEstate.realised).isAfter(dayjs()))}
            <span class="price-realised"
              >{fetchedEstate.type === 'flat'
                ? 'Квартира'
                : fetchedEstate.type === 'house'
                  ? 'Дом'
              ...
              fetchedEstate.type === 'commersion'
                ? 'сдана'
                : 'сдан'}{!isNaN(Date.parse(fetchedEstate.realised))
        ...
        {#if fetchedEstate.extras.included && fetchedEstate.extras.included[0] && fetchedEstate.type !== 'land'}
          <div class="extras-scroll-wrapper">
            {#each fetchedEstate.extras.included as extra}
              <div
                title={flatExtras.find((el) => el.value === extra)
                  ? flatExtras.find((el) => el.value === extra).label
                  : extra === 'hoz'
                    ? 'Хоз. постройки'
                    : extra === 'bas'
                      ? 'Бассейн'
                ...
                use:tooltip
                class="extra-icon"
                style="background-image: url(https://assets.protown.in.ua/extras-icos/{extra}.svg)"
              />
            {/each}
          </div>
        {/if}
        {#if fetchedEstate.details.communications && fetchedEstate.details.communications[0] && (fetchedEstate.type === 'land' || fetchedEstate.type === 'house')}
          <div class="extras-scroll-wrapper communications">
            {#each fetchedEstate.details.communications as extra}
              <div
                title={communicationsList.find((el) => el.value === extra)
                  ?.label}
                use:tooltip
                class="extra-icon"
                style="background-image: url(https://assets.protown.in.ua/communications-icons/{extra}.svg)"
              />
            {/each}
          </div>
        {/if}
        <div class="estate-info">
          {#if fetchedEstate.details.area && checkForObject(fetchedEstate.details.area)}
            <fieldset
              class="area-field"
              use:viewport
              on:enterViewport={({ target }) =>
                !target.className.includes('inView')
                  ? (target.className += ' inView')
                  : null}
            >
              ...
              <div class="area-poperties">
                {#each Object.keys(fetchedEstate.details.area) as areaProp}
                  <div class="area-prop">
                    ...
                    >
                    <span class="value"
                      >{fetchedEstate.details.area[
                        areaProp
                      ]}{#if areaProp !== 'whole'}м<sup>2</sup>
                      {:else}&nbspсоток{/if}</span
                    >
                  </div>
                {/each}
              </div>
              ...
              <div class="estate-detailed-poperties">
                {#if fetchedEstate.details.floor || fetchedEstate.details.gfloor}
                  <div class="estate-detailed-prop">
                    <span class="label"
                      >{!fetchedEstate.details.floor &&
                      fetchedEstate.details.gfloor
                        ? 'Этажность'
                        : 'Этаж'}</span
                    >
                    <span class="value">
                ...
                {#if fetchedEstate.details.rooms}
                  <div class="estate-detailed-prop">
                    <span class="label">Комнат</span><span class="value"
                      >{fetchedEstate.details.rooms}</span
                    >
                  </div>
                {/if}
                {#if fetchedEstate.details.sillings}
                  <div class="estate-detailed-prop">
                    <span class="label">Высота потолков</span><span
                      class="value">{fetchedEstate.details.sillings}м</span
                    >
                  </div>
                {/if}
                {#if fetchedEstate.details.purpose}
                  <div class="estate-detailed-prop">
                    <span class="label">Назначение</span><span class="value"
                      >{landTypes.find(
                        (el) => el.value === fetchedEstate.details.purpose
                      )?.label}</span
                    >
                  </div>
                {/if}
                {#if fetchedEstate.details.planning}
                  <div class="estate-detailed-prop">
                    <span class="label">Планировка</span><span class="value"
                      >{[
                        { value: 'open', label: 'Открытого типа' },
                        { value: 'close', label: 'Закрытого типа' },
                        ...flatPlanning,
                      ].find(
                        (el) => el.value === fetchedEstate.details.planning
                      )?.label}</span
                    >
                  </div>
                {/if}
                {#if fetchedEstate.type === 'land'}
                  <div class="estate-detailed-prop">
                    <span class="label"
                      >{fetchedEstate.deal === 'lease'
                        ? 'Аренда'
                        : 'Покупка'}&nbsp;частями</span
                    >
                    <span class="value"
                      >{fetchedEstate.details.partly ? 'Да' : 'Нет'}</span
                    >
                  </div>
                {/if}
              </div>
          ...
          >
            ...
            {#if fetchedEstate.adress.country}
              <div class="estate-location-prop">
                <span class="label">Страна</span><span class="value"
                  >{countries1.find(
                    (el) => el.value === fetchedEstate.adress.country
                  )?.label}</span
                >
              </div>
            {/if}
            {#if fetchedEstate.adress.city && checkForObject(fetchedEstate.adress.city)}
              <div class="estate-location-prop">
                <span class="label">Город</span><span class="value"
                  >{fetchedEstate.adress.city.ru
                    ? fetchedEstate.adress.city.ru
                    : fetchedEstate.adress.city[
                        Object.keys(fetchedEstate.adress.city)[0]
                      ]}</span
                >
              </div>
            {/if}
            {#if fetchedEstate.adress.district}
              <div class="estate-location-prop">
                <span class="label">Район</span><span class="value"
                  >{kyivDistricts.find(
                    (el) => el.value === fetchedEstate.adress.district
                  )?.label}</span
                >
              </div>
            {/if}
            {#if fetchedEstate.adress.street && checkForObject(fetchedEstate.adress.street)}
              <div class="estate-location-prop">
                <span class="label">Улица</span><span class="value"
                  >{fetchedEstate.adress.street.ru
                    ? fetchedEstate.adress.street.ru
                    : fetchedEstate.adress.street[
                        Object.keys(fetchedEstate.adress.street)[0]
                      ]}{fetchedEstate.adress.estateNumber
                    ? `, ${fetchedEstate.adress.estateNumber}`
                    : ''}</span
                >
              </div>
            ...
            {#if fetchedEstate.adress.zk && checkForObject(fetchedEstate.adress.zk)}
              <div class="estate-location-prop">
                <span class="label">Ж/К</span><span class="value"
                  >{fetchedEstate.adress.zk.ru
                    ? fetchedEstate.adress.zk.ru
                    : fetchedEstate.adress.zk[
                        Object.keys(fetchedEstate.adress.zk)[0]
                      ]}</span
                >
              </div>
            {/if}
            {#if fetchedEstate.adress.metro && checkForObject(fetchedEstate.adress.metro)}
              <div class="estate-location-prop">
                <span class="label">Метро</span><span class="value"
                  >{fetchedEstate.adress.metro.ru
                    ? fetchedEstate.adress.metro.ru
                    : fetchedEstate.adress.metro[
                        Object.keys(fetchedEstate.adress.metro)[0]
                      ]}{fetchedEstate.adress.metro.distance
                    ? ` в ${fetchedEstate.adress.metro.distance} метрах`
                    : ''}</span
                >
              </div>
            {/if}
        ...
        <fieldset class="estate-comment-field">
          <legend>
            <div class="legend-content-wrapper">
              <div
                class="avatar"
                style="background-image: url(https://assets.protown.in.ua/avatars/default_min.jpg);"
              />
              <div class="rieltor">
                ...
                <div style="display: flex;">
                  ...
                  <a
                    class="rieltor-viber"
                    href="viber://contact?number=%2B38{fetchedRieltor.mobile}"
                  >
                    <div
                      class="icon"
                      style="background-image: url(/viber.svg)"
                    />
          ...
          <div class="comment-wrapper">
            <span class="comment"
              >{fetchedEstate.extras.comment.ru
                ? fetchedEstate.extras.comment.ru
                : fetchedEstate.extras.comment[
                    Object.keys(fetchedEstate.extras.comment)[0]
                  ]}</span
            >
          </div>
        </fieldset>
...
<style>
  ...
  .area-poperties,
  .estate-detailed-poperties {
    margin-top: 0.5em;
  }
  .estate-location-prop,
  .area-prop,
  .estate-detailed-prop {
    display: flex;
    margin: 0 auto;
