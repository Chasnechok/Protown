...
<div class="dragdroplist">
    ...
    <div 
        ...
        {#each data as datum, i (datum.id ? datum.id+i : JSON.stringify(datum)+i)}
            <div 
                id={(grabbed && (datum.id ? datum.id+i : JSON.stringify(datum)+i) == grabbed.dataset.id) ? "grabbed" : ""}
                class="item"
                data-index={i}
                data-id={(datum.id ? datum.id+i : JSON.stringify(datum)+i)}
                data-grabY="0"
                ...
                <div class="buttons">
                    <button
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="30px" height="30px"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M7.41 15.41L12 10.83l4.59 4.58L18 14l-6-6-6 6 1.41 1.41z"/></svg>
                    </button>
                    <button
                        type="button" 
                        class="down" 
                        ...
                        on:click={function(ev) {moveDatum(i, i + 1)}}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="30px" height="30px"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z"/></svg>
                    </button>
                </div>
                ...
                <div class="buttons delete">
                    {#if removesItems}
                        <button
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" height="22" viewBox="0 0 24 24" width="22"><path d="M0 0h24v24H0z" fill="none"/><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>
                        </button>
                    {/if}
                </div>
            </div>
